...
- `databaseUrl` and `osUsername` are computed for all modes but only rendered where templates reference them (Postgres-only branches). A harmless unused value in `templateData()` is not a bug — do not flag it.
